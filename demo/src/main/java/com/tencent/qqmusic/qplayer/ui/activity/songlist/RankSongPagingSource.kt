...
class RankSongPagingSource(val rankId: Int) : PagingSource<Int, SongInfo>() {

    companion object {
        private const val TAG = "RankSongPagingSource"
    }
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SongInfo> {
        return try {
            withContext(Dispatchers.IO) {
                ...
                val songList = OpenApiSDK.getOpenApi().blockingGet<List<SongInfo>> {
                    OpenApiSDK.getOpenApi().fetchSongOfRank(rankId, nextPage, params.loadSize, callback = it)
                }.data ?: emptyList()
                    ?: emptyList()
                val prevKey = if (nextPage == 0) null else nextPage - 1
                val nextKey = if (songList.isEmpty()) null else nextPage + 1
                Log.i(TAG,
                    "load: rank id $rankId, next page $nextPage, prev key $prevKey, next key $nextKey")
                LoadResult.Page(
                    data = songList,
                    prevKey = prevKey,
                    nextKey = nextKey
