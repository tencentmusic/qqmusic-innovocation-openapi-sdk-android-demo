 ...
 */
class OrderedSingerPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Singer> = try {
        ...
        withContext(Dispatchers.IO) {
            var hasMore = false
            val response = OpenApiSDK.getOpenApi().blockingGet<List<Singer>> {
            ...
            response.totalCount?.let(onTotalCountChanged)
            val singers = response.data ?: emptyList()
            val prevKey = if (nextPage == 0) null else nextPage - 1
            val nextKey = if (hasMore) nextPage + 1 else null
            return@withContext LoadResult.Page(singers, prevKey, nextKey)
        }
    } catch (e: Exception) {
        QLog.i("OrderedSinger", "[load] load collected singer exception.", e)
        LoadResult.Error(e)
    }
}
